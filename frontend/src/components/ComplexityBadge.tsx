...
export const ComplexityBadge: React.FC<ComplexityBadgeProps> = ({
  ...
  const colorClass =
    ...
    "bg-gray-100 dark:bg-gray-900/30 text-gray-800 dark:text-gray-300 border-gray-200 dark:border-gray-700";
  const rating = complexityRating[
    complexity as keyof typeof complexityRating
  ] || { rating: "Unknown", icon: "❓" };

  return (
    <div className="space-y-2">
      <div
        className={`inline-flex items-center px-4 py-2 rounded-lg border font-mono text-lg font-semibold transition-colors duration-200 ${colorClass}`}
      >
        {complexity}
      </div>
